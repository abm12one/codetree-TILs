...
vector<vector<tuple<int,int,int>>>road;
int sn;
...
{
	sn=N;
	dist=vector<vector<vector<int>>>(301,vector<vector<int>>(31,vector<int>(31,987654321)));
	mindist=vector<vector<vector<int>>>(301,vector<vector<int>>(31,vector<int>(31,987654321)));
	road=vector<vector<tuple<int,int,int>>>(34,vector<tuple<int,int,int>>(34,make_tuple(-1,0,0)));
	for(int i=0;i<K;i++){
		int na=mNodeA[i]/100;
		int ac=mNodeA[i]%100;
		int nb=mNodeB[i]/100;
...
{	
	...
	while(!pq.empty()){
		...
		pq.pop();
		//cout<<mind[now]<<" "<<nowd<<"\n";
		if(visit[now]||mind[now]<nowd)continue;
		visit[now]=1;
		//cout<<"wow"<<"\n";
		int nowcity=0;
